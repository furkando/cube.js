...
export function makeCubeSchema({ refreshKey, preAggregations }) {
  return ` 
      cube('cards', {
        sql: \`
        select * from cards
...
describe('Base Query', () => {
  describe('Common', () => {
    const { compiler, joinGraph, cubeEvaluator } = prepareCompiler(makeCubeSchema({
      refreshKey: '',
      preAggregations: ''
    }));
  ...
  describe('RefreshKey with every', () => {
    const { compiler, joinGraph, cubeEvaluator } = prepareCompiler(makeCubeSchema({
      refreshKey: `
        refreshKey: {
          every: '1 hour',
          sql: 'SELECT MAX(created_at) FROM cube',
        },
      `,
      preAggregations: `
        maxCreatedAt: {
    ...
    it('cacheKeyQueries for cube with refreshKey.every (source)', async () => {
      ...
      expect(query.cacheKeyQueries()).toEqual([
        [
          // Postgres dialect
          'SELECT MAX(created_at) FROM cube',
          [],
          {
            // false, because there is no externalQueryClass
            external: false,
    ...
    it('preAggregationsDescription for query', async () => {
      await compiler.compile();
